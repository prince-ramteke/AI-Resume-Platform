...
- **v1.2.M1 — Retrieval evaluation harness + retrieval observability.** ✅ Testcontainers-backed harness that compares vector-only vs. hybrid retrieval on a labeled fixture set (`backend/src/test/resources/rag-eval/cases/*.json`) using Recall@K (K = 3, 5, 8) and MRR; deterministic (`FakeEmbeddingClient`), no LLM/Ollama calls. Adds retrieval-side Micrometer metrics — `rag.retrieval.latency` (per arm + mode), `rag.retrieval.candidates` (per arm), `rag.retrieval.overlap`, `rag.retrieval.fusion.winner`, `rag.retrieval.fusion.contribution`, `rag.retrieval.dropped` (`reason={topk, token_budget}`) — with strictly bounded enum-like tag values (see `docs/SECURITY.md §2`). Report written to `target/rag-eval-report.txt` for humans; no CI quality gate. Hybrid default stayed **off** (`RAG_HYBRID_ENABLED=false`) pending M2 evidence.

- **v1.2.M2 — Retrieval quality decision & tuning.** ✅ Turns the M1 harness into an evidence-backed decision on the hybrid default. Fixture set expanded from 5 to 15 synthetic cases covering acronyms, version/stack matching, framework synonyms, cloud/DevOps, database internals, education/certifications, leadership impact, frontend, and adversarial keyword distractors. Harness extended to write per-case Recall@K + RR and the aggregated retrieval observability meters (candidates, overlap, fusion winner/contribution, drop counters, per-arm-per-mode latency) into `target/rag-eval-report.txt`. New `RetrievalTuningIT` sweeps `rrf_k ∈ {30, 60, 90}` × `pool ∈ {10, 20, 40}` (nine combinations) into `target/rag-eval-tuning-report.txt`. All results, RRF sweep, and the final decision are recorded in [`docs/eval/retrieval-report.md`](eval/retrieval-report.md). **v1.2.M3 (production fix + final enable):** `KeywordQueryBuilder` extracts ≤5 technical tokens from the full JD text before passing to `plainto_tsquery`, fixing the production bug where the keyword arm always returned 0 candidates; production-aligned re-evaluation (15 JD-sentence queries) showed Recall@3 +13.33pp, MRR +23.89pp, zero regressions. `RAG_HYBRID_ENABLED` default **flipped to `true`** in `application.yml` on 2026-08-13. **No new production API, no CI-executed LLM calls, no changes to `ChunkEvidence` or `AnalysisResponse`.**
