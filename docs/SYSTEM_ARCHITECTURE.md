...
**Hybrid retrieval (v1.1/v1.2):** step 4 blends the vector arm with a PostgreSQL full-text (keyword) arm over the same `document_chunks.content`, scoped to the same `(source_type, source_id)`. The query text (JD) is first processed by `KeywordQueryBuilder` (v1.2.M2), which extracts ≤5 distinctive technical tokens (uppercase heuristic + curated TECH_VOCAB) before passing them to `plainto_tsquery`; if no technical tokens are found the keyword arm is skipped entirely (no DB round-trip). Each arm returns a candidate pool (`app.rag.hybrid-candidate-pool-size`, default 20); the two rankings are fused by **Reciprocal Rank Fusion** — for a candidate at 1-based rank `r`, contribution `1/(k + r)` with `k = app.rag.hybrid-rrf-k` (default 60), summed across arms — and the top-k survive. The "re-rank" is this deterministic, rank-based fusion; there is **no** ML/cross-encoder re-ranker. A chunk that only the keyword arm surfaces can still reach the prompt, improving recall for exact-term matches (tools, acronyms) that embeddings blur. **Enabled by default** (`app.rag.hybrid-enabled=true`) as of v1.2.M2 (2026-08-13): 15-case production-aligned evaluation showed Recall@3 +13.33pp, MRR +23.89pp, zero regressions. Requires the `V6` full-text index (shipped in v1.1). Evidence shape (`ref`, `snippet`, `sourceType`, `chunkIndex`) is identical in both modes, so grounding, citation, and the `AnalysisResponse` contract are unchanged.
...
| **Observability** | Micrometer metrics — analysis count (by result + cache), analysis latency, LLM latency (by provider), LLM token usage (by provider + type) — exported at `/actuator/prometheus` and visualized in Grafana (v1.1); `/actuator/health`; trace id per request in logs. Actuator runs on a separate, unpublished management port (`9091`) scraped by Prometheus over the internal network (see DEPLOYMENT.md §8). **Retrieval observability (v1.2.M1):** `rag.retrieval.latency` (tags `arm={vector\|keyword\|fuse\|total}`, `mode={vector\|hybrid}`), `rag.retrieval.candidates` (tag `arm={vector\|keyword}`), `rag.retrieval.overlap` (no tags; hybrid only — size of the chunk-index intersection between the two pools), `rag.retrieval.fusion.winner` (tag `arm={vector\|keyword\|both}` — origin of the top-1 hybrid result), `rag.retrieval.fusion.contribution` (tag `arm={vector\|keyword\|both}` — count of final top-k results attributable to each arm), `rag.retrieval.dropped` (tag `reason={topk\|token_budget}`). All retrieval tag values are fixed enum-like strings — never ids, query text, snippets, or document content. Instrumentation is purely additive: candidate ordering, scores, and the returned evidence list are unchanged. |
...
| Retrieval | Vector top-k in v1; hybrid (vector + keyword, RRF fusion) on by default from v1.2.M2 | 15-case eval: Recall@3 +13.33pp, MRR +23.89pp, zero regressions; `KeywordQueryBuilder` bounds the FTS query to distinctive technical tokens | Add a learned re-ranker if RRF proves insufficient. |
